...
if exists("b:is_bash")
 ...
 syn keyword bashStatement add-apt-repository apt apt-cache apt-file apt-get apt-key atom atom-beta atom-git awk bundle bzcat cat chmod chroot clear code code-oss complete cp dnf dpkg du egrep emacs expr fgrep find gnufind gnugrep grep gunzip gvim gvim-gtk2 gvim-gtk3 gzip less ln ls mkdir mount mv nano pacman pkg pip pip2 pip3 rc-add rm rmdir rpm sed sleep sort strip systemctl tail urpme urpmf urpmi urpmi_rpm-find-leaves@ urpmq vi vim yum yum-builddep zcat zypper
